...
trainingData <- data.matrix(read.table("/Users/tjordan/Desktop/ML/Data/housing_train.txt"))
...
XTest <- data.matrix(testData[ ,1:numFeatures])
YTest <- data.matrix(testData[ ,numFeatures + 1])
PredictedYTest <- predictLabels(XTest, bestWeights)
TestError <- computeError(YTest, PredictedYTest)
